...
${patterns.map((p) => `- **${p.disease}**: Cluster of ${p.clusterSize} (Members: ${p.affectedMembers?.join(", ")}). Weight: ${p.inheritanceWeight}`).join("\n")}

## Personal Risk Assessment (Genetic Load Map)
${riskScores.map((r) => `### ${r.disease} - RISK LEVEL: **${r.level.toUpperCase()}** (Score: ${r.score}%)
- **Clinical Evidence:** ${r.evidence}
- **Recommended Actions:**
  ${r.prevention?.map((tip) => `1. ${tip}`).join("\n  ")}`).join("\n\n")}
